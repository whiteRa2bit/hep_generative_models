...
from generation.dataset.dataset import get_attributes_df, get_detector_event_df, get_detector_event_df_path, \
...
def _prepare_event_df(df_full, event: int, df_dir: str = DF_DIR):  # TODO: (@whiteRa2bit, 2020-07-21) Add documentation
    ...
    :return: 
    """
    event_df = detector_df[detector_df['event'] == event]  # TODO: (@whiteRa2bit, 2020-08-25) Fix call by attribute name
    detectors = detector_df['detector'].unique()  # TODO: (@whiteRa2bit, 2020-08-25) Fix call by attribute name    
...
def main(): 
    ...
    for event in events:
        ...
        for detector in tqdm.tqdm(detectors):
            _prepare_detector_event_signal(detector, signal)
